...
### 12.3 线上实测（push 后 90 秒执行）

| URL | 状态码 | 期望 | 结论 |
|---|---|---|---|
| `/udrp/` | 200 | 200 | ✓ 且「线上抓取的 title 与 H1 均为新版」已确认 |
| `/udrp/cases/` | 200 | 200 | ✓ 且 `robots` 线上实测为 `noindex,follow` |
| `/udrp/cases/_templates/case.html` | **404** | 404 | ✓ 下划线目录排除机制再次线上实证 |
| `/services/ai-domain-disputes.html` | 200 | 200 | ✓ |
| `/en/udrp-counsel.html` | 200 | 200 | ✓ |
| `/articles/udrp-can-brand-owner-recover-domain.html` | 200 | 200 | ✓ 且面包屑 HTML 线上确认已生效 |
| `/sitemap.xml` | 200 | 200 | ✓ 且不含 `udrp/cases`；`/udrp/` lastmod = 2026-09-12 |
| `/_internal/udrp-规则核对记录.md` | 404 | 404 | ✓ |
| `/_internal/udrp-cases/schema.json` | 404 | 404 | ✓ |
| `/_internal/UDRP内容中枢V1-验收报告.md` | 404（5/5 轮） | 404 | ✓ |
| `/` | 200（5 轮中 3 轮 200，2 轮 `000`） | 200 | ✓ |
| `/practices.html`、`/llms.txt` | 200 / 200 | 200 | ✓ |

**关于 `000`**：按本项目的验收纪律，`000` 是连接层失败，**不等于 404**，必须复测到明确状态码。首页与内部报告两处出现 `000` 后各复测 5 轮，首页得到 200（说明是链路抖动）、内部报告 5/5 稳定 404（说明确已排除）。若把 `000` 直接读成"已下线"，会得出错误结论。

### 12.4 修改范围自查（对照简报第十一节）
